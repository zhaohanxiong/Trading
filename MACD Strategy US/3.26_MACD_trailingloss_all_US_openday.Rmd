...
macd_strategy = function(dat_i) {
  ...
  for (m in macd_entry) {
    ...
    # exit if price is below price_entry, suggesting less chance of MACD crossover
    if (dat_i$open[m] <= price_entry) next
    
    # set stop price, if stop loss is fked and above entry, then skip trade
    price_stop   = dat_i$low[dat_i$date == min_date][1]
    ...
    # store trade if entry price is actually hit as limit order intraday
    # store trade if there is an exit, and if the exit date exists
...
trade_hist1 = trade_hist1[trade_hist1$ticker %in% stocks$Symbol[stocks$Cap>500],]
...
abline(h=-1:20*100,col=alpha("grey",0.5))
...
> Strategy 1: MACD
  ...
  - Validate MACD cross 1 day problem
      ...
      - only enter MACD crosses during backtesting if MACD statisfies a certain MACD action condition
