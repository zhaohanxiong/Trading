...
# Load data
nasdaq = gsub(".csv","",list.files(file.path(Sys.getenv("USERPROFILE"),"Desktop","nasdaq stocks")))
...
for (i in 1:length(nasdaq)) {
  ...
  s = googlefinance_single(nasdaq[i], "NASDAQ")
...
xlsx_names = c()
for (i in 1:length(nasdaq)) {
  xlsx_names = c(xlsx_names, c(nasdaq[i],rep("",5)))
}
...
for (i in seq(1,ncol(all_dat),by=6)) {
  ...
  if (!is.null(col1)) {
    ...
    # load stock
    edit_dat   = read.csv(paste0("nasdaq stocks/",edit_ticker,".csv"),sep=",",header=TRUE,stringsAsFactors=FALSE)
    ...
    if (current_last_day < updated_first_day) {
      ...
      write.csv(write_dat,paste0("nasdaq stocks/",edit_ticker,".csv"),row.names=FALSE)
...
```{r}
setwd(file.path(Sys.getenv("USERPROFILE"),"Desktop","nasdaq stocks"))

ff = list.files()

for (f in ff) {
  
  dat      = read.csv(f,sep=",",header=TRUE,stringsAsFactors=FALSE)
  dat$date = as.Date(dat$date,format="%Y-%m-%d")
  dat = dat[dat$date != as.Date("2021-02-05"),]
  
  write.csv(dat,f,row.names=FALSE)
