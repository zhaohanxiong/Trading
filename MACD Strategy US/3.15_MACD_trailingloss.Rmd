...
macd_strategy = function(dat_i) {
  ...
  for (m in macd_entry) {
    ...
    price_stop   = dat_i$low[dat_i$date == min_date]*0.99 # put stop loss %0.5 below prior swing low
    ...
    if (price_entry >= dat_i$low[m] & price_entry <= dat_i$high[m])  {
      if (is.finite(exit_date) & !is.na(dat_i$date[exit_date])) {
        ...
        if (price_exit > dat_i$open[exit_date]) {
          price_exit = dat_i$open[exit_date]
        }
...
if (exchange == "nysearca") {
  trade_hist1 = trade_hist1[trade_hist1$ticker %in% stocks$Symbol[stocks$Annualized_Return > 5],]
} else {
...
- test which stocks have entry but no exit (i.e. currently trade is happening)

- algorithm to determine which trades to take (cut current trade if a way better one comes along?)

    - AT THE VERY END to tune MACD strategy: 
