...
macd_strategy = function(dat_i) {
  ...
  macd_entry = which(macd_cross & macd12_26_9$MACD < 0) + 1
  ...
  for (m in macd_entry) {
    ...
    price_entry = 351/28*(28/351* dat_i$Close[m-2] + 11/13*(ema12[m-3] - ema12[m-2]) + 25/27*(ema26[m-2] - ema26[m-3]))
    ...
    # if low of MACD cross is greater than entry day low, then exit on entry day
    if (!(is.finite(exit_date) & !is.na(dat_i$Date[exit_date]))) next
    if (dat_i$Low[m-1] > dat_i$Low[m]) exit_date = m
    
    # compute exit price
    if (exit_date == m) {
      price_exit = dat_i$Low[m-1]
    } else {
...
p2[[4]] = sprintf("Proportion of Days in Position: %0.1f%%",
                  as.numeric(sum(trade_hist1$exit.date - trade_hist1$entry.date + 1)/
...
for (i in 4000:nrow(trade_hist1)) {
