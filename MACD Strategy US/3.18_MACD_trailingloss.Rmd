...
# Find Next Trade
```{r}
next_trades = data.frame(ticker=as.character(),entry.date=as.character(),
                         entry.price=as.numeric(),stop.price=as.numeric())
...
for (i in 1:nrow(stocks)) {
    ...
    # most recent entry, and compute stop loss
    macd_entry = which(macd_cross & macd12_26_9$MACD < 0 & df$close > ema200)
    if (length(macd_entry) < 1) {
      next
    } else {
      
      m = tail(macd_entry,1)
      if (m < (nrow(df) - 10)) next
      ...
      # define exit date
      next_trades = rbind(next_trades,c(stocks$Symbol[i],df$date[m],price_entry,price_stop))
...
# define column names
colnames(next_trades) = c("Symbol","EntryDate","EntryPrice","StopPrice","StopPercentage")
next_trades$EntryPrice = round(as.numeric(next_trades$EntryPrice),3)
next_trades$StopPrice  = round(as.numeric(next_trades$StopPrice),3)
```

> Strategy 1: MACD
  - find which stocks will produce the next signal
