...
# Load data
nysearca = gsub(".csv","",list.files(file.path(Sys.getenv("USERPROFILE"),"Desktop","nysearca stocks")))

# loop through tickers and generate excel formula
xlsx = c()
for (i in 1:length(nysearca)) {
  
  # input with ticker and exchange symbols
  s = googlefinance_single(nysearca[i], "NYSEARCA")
...
xlsx_names = c()
for (i in 1:length(nysearca)) {
  xlsx_names = c(xlsx_names, c(nysearca[i],rep("",5)))
}
...
for (i in seq(1,ncol(all_dat),by=6)) {
  ...
  if (!is.null(col1)) {
    ...
    # load stock
    edit_dat   = read.csv(paste0("nysearca stocks/",edit_ticker,".csv"),sep=",",header=TRUE,stringsAsFactors=FALSE)
    ...
    if (current_last_day < updated_first_day) {
      ...
      write.csv(write_dat,paste0("nysearca stocks/",edit_ticker,".csv"),row.names=FALSE)
