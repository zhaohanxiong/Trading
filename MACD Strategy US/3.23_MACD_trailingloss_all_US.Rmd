...
macd_strategy = function(dat_i) {
  ...
  # ------------------------------------------------------------------------------------------------
  # Parameters To Tune:
  # - 200 EMA filter or not                              (no filter is best)
  ...
  for (m in macd_entry) {
    ...
    if (price_entry >= dat_i$low[m] & price_entry <= dat_i$high[m])  {
      if (is.finite(exit_date) & !is.na(dat_i$date[exit_date])) {
        ...
        # compute exit price
        price_exit = max(c(dat_i$high[m:(exit_date-1)]))*(100+stop_per)/100
...
osc = sapply(1:length(macd_trade[-1]),function(x) ifelse(x <= round(n_trades_yr,0),
