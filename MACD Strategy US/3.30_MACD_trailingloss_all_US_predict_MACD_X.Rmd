...
for (i in 1:dim(MACD_diff)[2]) {
  MACD_diff[,i] = order(MACD_diff[,i],decreasing=FALSE)
}

# only use top 10
MACD_diff[MACD_diff>10] = NA
```
...
macd_strategy = function(dat_i,ind_i) {
  ...
  # return if data is less than 100 in length and cant compute 200ema
  if (nrow(dat_i) <= 100) return(empty_out)
  ...
  # entry
  macd_entry = unname(which(!is.na(MACD_diff[ind_i,]))) - (8000 - nrow(dat_i)) + 1
  ...
  for (m in macd_entry) {
    ...
    # set stop price, if stop loss is fked and above entry, then skip trade
    price_stop = dat_i$Close[dat_i$Date == min_date][1] # Use Low or Close
    if (price_stop > price_entry) next
    ...
    # if macd correctly crossed the next day
    if (macd12_26_9$signal[m-1] > macd12_26_9$MACD[m-1] & macd12_26_9$signal[m] < macd12_26_9$MACD[m]) {
    ...
    } else { # MACD did not cross, then use at the close order to get out of trade
...
> Strategy 1: MACD
  - implement new method of MACD entry with the top 10 most likely entries instead of MACD crosses
  - implement early exit if there is no crossover next day based on min MACD diff
