...
macd_strategy = function(dat_i) {
  ...
  macd_sig  = as.numeric(c())
  macd_macd = as.numeric(c())
  ...
  for (m in macd_entry) {
    ...
    if (price_entry >= dat_i$Low[m] & price_entry <= dat_i$High[m])  {
      if (is.finite(exit_date) & !is.na(dat_i$Date[exit_date])) {
        ...
        macd_sig  = c(macd_sig,macd12_26_9$signal[m])
        macd_macd = c(macd_macd,macd12_26_9$MACD[m])
  ...
  dat_out = data.frame(entry.date   = as.Date(out_date_entry),
                       ...
                       macd.signal = as.numeric(macd_sig),
                       macd.macd   = as.numeric(macd_macd))
...
trade_hist = data.frame(ticker=character(),exchange=character(),
                        ...
                        macd.signal=as.numeric(),macd.macd=as.numeric())
...
> Strategy 1: MACD
  - Validate MACD cross 1 day problem
      - test for every day, how many highest potential entries has macd cross in 1d
      - test for every day, how many highest potential entries has macd cross in 1 day AND open price is already cross
      - for all MACD entries, see the MACD action before entry and summarize statistics
      - only enter MACD crosses during backtesting if MACD statisfies a certain MACD action condition which guarantees cross in next day
      - test given a certain MACD action, what is the likelihood it will cross in 1 day, and does open price make a difference
