...
macd_strategy = function(dat_i) {
  ...
  for (m in macd_entry) {
    ...
    if (is.finite(exit_date) & !is.na(dat_i$date[exit_date])) {
      ...
      price_exit = max(dat_i$close[m:exit_date])*(100+stop_per)/100
...
- improve MACD entry by calculating the exact price required to trigger MACD crossover, then use this as entry price
