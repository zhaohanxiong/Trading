...
# ----------------------------------------------------------------------------------------------------------------------
# compute EMA filters
ema_filter = function(emaX,price,param_diff,param_grad) {
  
  # find places where ema is close to price
  cond1 = abs((emaX - price)/emaX*100) < param_diff
  
  # find places where gradient of ema is flat
  cond2 = abs(c(0,diff(emaX))/emaX*100) < param_grad
