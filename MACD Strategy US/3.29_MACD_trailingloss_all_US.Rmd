...
# Initialization
```{r}
MACD_diff = as.matrix(read.csv(file.path(Sys.getenv("USERPROFILE"),"Desktop","MACD_diff.csv")))
...
macd_strategy = function(dat_i,ind_i) {
  ...
  if (nrow(dat_i) <= 125) return(empty_out)
  ...
  macd_entry = unname(which(!is.na(MACD_diff[ind_i,]))) - (8000 - nrow(df))
  macd_entry = macd_entry[macd_entry>28]
  ...
  for (m in macd_entry) {
    ...
    # store the trade if there is an exit, and if the entry price is hit intra-day
