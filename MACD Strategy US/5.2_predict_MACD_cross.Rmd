...
# Notes:
# - Using top 20 most likely MACD_diff produces the best annual return
# - Winning % does not differ amongst the 1st to 20th most likely values
# - ~50% of trades actually have MACD cross, the crossed trades have ~75% win, the non-crossed have ~35% win
# - using correct cross is better if only 50% of trades have MACD cross, and it produces more consistent entries

# Pre-Compute MACD Stats
```{r}
rm(list=ls())
source("Utils.R")
...
for (i in 1:nrow(stocks)) {
    ...
    # store values
    macd_mat[i, (8000 - nrow(df) + 1):8000,1] = macd_cross
    macd_mat[i, (8000 - nrow(df) + 1):8000,2] = macd12_26_9$signal
    macd_mat[i, (8000 - nrow(df) + 1):8000,3] = macd12_26_9$MACD

}

# compute difference in MACD line and signal line
MACD_diff = abs((macd_mat[,,2] - macd_mat[,,3])/macd_mat[,,2])
MACD_diff[macd_mat[,,2] >= 0] = NA
MACD_diff[macd_mat[,,3] >= 0] = NA
MACD_diff[macd_mat[,,2] <= macd_mat[,,3]] = NA
MACD_diff[!is.finite(MACD_diff)] = NA
MACD_diff[stocks$Cap < 1000,] = NA

# change MACD diff values into rankings based on smallest number
for (i in 1:dim(MACD_diff)[2]) {
  if (sum(!is.na(MACD_diff[,i])) > 20) {
    MACD_diff[,i] = rank(MACD_diff[,i],ties.method="first")
  } else {
    MACD_diff[,i] = NA
  }
}
...
# MACD Strategy
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
macd_strategy = function(dat_i,ind_i) {

  # ------------------------------------------------------------------------------------------------
  # Parameters To Tune:
  # - special back-computed MACD entry                   (real cross is better)
  # - % to skip trade if stop loss X% is too large       (2% is best)
  # - Use prior Close/Low to identify min of pullback    (Close vs Low, Low is better)
  # - Use prior Close/Low of pullback for stop loss      (Close vs Low, Low is better)
  ...
  # entry
  macd_entry = unname(which(!is.na(MACD_diff[ind_i,(8000 - nrow(dat_i) + 1):8000]))) + 1
  macd_entry = macd_entry[macd_entry > 35]
  macd_entry = macd_entry[macd_entry < nrow(dat_i)]
  ...
  out_diff_order   = as.numeric(c())
  out_macd_cross   = as.numeric(c())
  ...
  for (m in macd_entry) {
    ...
    price_stop = dat_i$Low[dat_i$Date == min_date][1] # Use Low or Close
    ...
    out_diff_order = c(out_diff_order,MACD_diff[ind_i,][m + (8000 - nrow(dat_i)) - 1])
    out_macd_cross = c(out_macd_cross,macd12_26_9$signal[m-1] > macd12_26_9$MACD[m-1] & macd12_26_9$signal[m] < macd12_26_9$MACD[m])
  ...
  dat_out = data.frame(entry.date   = as.Date(out_date_entry),
                       ...
                       macd.order   = as.numeric(out_diff_order),
                       macd.cross   = as.numeric(out_macd_cross))
...
trade_hist = data.frame(ticker=character(),exchange=character(),
                        ...
                        macd.order=as.numeric(),macd.cross=as.numeric())
...
# produce more statistics for the MACD_diff
sprintf("Proportion of Actual MACD Cross: %.1f%%",sum(trade_hist$macd.cross)/nrow(trade_hist)*100)
sprintf("Winning %% for MACD Crosses: %.1f%%",sum(trade_hist$outcome[trade_hist$macd.cross==1]>0)/sum(trade_hist$macd.cross==1)*100)
sprintf("Winning %% for non MACD Crosses: %.1f%%",sum(trade_hist$outcome[trade_hist$macd.cross==0]>0)/sum(trade_hist$macd.cross==0)*100)

#for (i in 1:20) {
#  print(sprintf("Winning %% for MACD rank %.0f: %.1f%%",
#                i,sum(trade_hist$outcome[trade_hist$macd.order==i]>0)/sum(trade_hist$macd.order==i)*100))
#}
