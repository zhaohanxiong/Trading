...
# TICKER   | WIN RATE |  Risk Reward  | Real Cum Return | Notes
# ------------------------------------------------------ trailing loss
# NASDAQ      36.7%          2.2            4488.3%
# NYSE       
# NYSEARCA   
...
plot(as.Date(sort(trade_hist1$entry.date)),macd_trade,type="l",log="y")
abline(h=seq(0,100,by=1),col=alpha("grey",0.5))

# portfolio statistics
drawdowns = sapply(1:length(macd_trade), function(x) (macd_trade[x] - max(macd_trade[1:(x-1)]))/max(macd_trade[1:(x-1)])*100)
rle       = rle(drawdowns<0)
p = list()
p[[1]] = sprintf("----------------------------------------------- Portfolio")
p[[2]] = sprintf("Maximum drawdown: %.1f%%", min(drawdowns))
p[[3]] = sprintf("Longest Porfolio Down Streak: %i Trades To Recover",max(rle$lengths[rle$values==TRUE]))
cat(paste0(unlist(p),collase="\n"))
```
