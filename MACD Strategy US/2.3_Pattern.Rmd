...
strategy_1 = function(data,entries,R,long=TRUE) {
...
# # ----------------------------------------------------------------------------------------------
# # evaluate long strategy based on a entry signals
# strategy_2 = function(data,entries,R,long=TRUE) {
# 
#   # max time to hold the position
#   max_hold = 100
#   
...
#       return = -R
#     } 
#     # hits first take profit target
#     else {
#       
#       # define 2nd bracket trade
#       if (long) {
#         bracket_prof2 = which(data$close[(ii+bracket_prof1-1):(ii+max_hold)] >= (data$close[ii]*(1 + 2.0*R/100))) # 2nd profit target
#         bracket_loss2 = which(data$close[(ii+bracket_prof1-1):(ii+max_hold)] <= data$close[ii])                   # 2nd stop loss
#       } else {
#         bracket_prof2 = which(data$close[(ii+bracket_prof1-1):(ii+max_hold)] <= (data$close[ii]*(1 - 2.0*R/100))) # 2nd profit target
#         bracket_loss2 = which(data$close[(ii+bracket_prof1-1):(ii+max_hold)] >= data$close[ii])                   # 2nd stop loss
#       }
#       
#       bracket_prof2 = ifelse(length(bracket_prof2)==0,max_hold,bracket_prof2[1])
#       bracket_loss2 = ifelse(length(bracket_loss2)==0,max_hold,bracket_loss2[1])
#       
#       # hits second stop loss (break even point)
#       if (bracket_loss2 <= bracket_prof2) {
#         exit   = (ii+bracket_prof1-1) + bracket_loss2 - 1
#         p_exit = data$close[ii]
#         return = 0.25*R #0.5*R
#       } 
#       # takes second take profit target
#       else {
#         exit   = (ii+bracket_prof1-1) + bracket_prof2 - 1
#         p_exit = data$close[ii]*(1 + 2.0*R/100*ifelse(long,1,-1))
#         return = 1.75*R #1.5*R
#       }
#     }
# 
#     # define output to append to sumamry
#     temp_row[1] = as.character(data$date[ii])        # entry date
#     temp_row[2] = data$close[ii]                     # entry price
#     temp_row[3] = as.character(data$date[exit])      # exit date
#     temp_row[4] = p_exit                             # exit price
...
#   out[[1]] = sprintf("Winning Trades: %.0f/%.0f",   sum(summary$return >= 0),nrow(summary))
#   out[[2]] = sprintf("Winning Percentage: %0.1f%%", sum(summary$return >= 0)/nrow(summary)*100)
#   out[[3]] = sprintf("Cumulative Return: %0.1f%%",  (cumprod(1+summary$return/100)[nrow(summary)]-1)*100)
#   out[[4]] = sprintf("Average Position Time Length: %0.1f Days", mean(summary$length))
#   
#   # write summary to file
#   txt = paste0("*****",ifelse(long," Long "," Short "),"*****\n")
#   txt = paste0(txt,paste(unlist(out),collapse="\n"),"\n")
...
# plot
plot_stock(ticker_symbol = t
          ,ticker_info   = nasdaq_filter$info[nasdaq_filter$Symbol==t]
          ,data          = t_df
          ,emas          = c(50,200)
          )
...
#long_entry  = which(macd_cross & macd12_26_9$MACD < 0 & t_df$close > ema200)
long_entry  = which(macd_cross & macd12_26_9$MACD < 0 & ema50 > ema200)

macd_cross  = crossover(macd12_26_9$MACD,macd12_26_9$signal)
short_entry = which(macd_cross & macd12_26_9$MACD >= 0 & t_df$close <= ema200)

# evaluate return
long_trade_hist = strategy_1(data=t_df,entries=long_entry,R=10,long=TRUE)
```
...
# BUY THE DIP DURING UPTREND
# PRICE ACTION BEFORE LARGE INCREASE/UPTREND
#
#
#
```
