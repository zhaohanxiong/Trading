...
macd_strategy = function(dat_i) {
  ...
  # ------------------------------------------------------------------------------------------------
  # Parameters To Tune:
  # - trailing stop loss or fixed stop loss                                     (trailing better)
  # - Use 200 EMA as filter                                                     (better without)
  # - % below pullback to set stop loss                                         (0% is best)
  # - % to skip trade if stop loss X% is too large                              (2.5% is best)
  # - % to skip trade if price has already moved X% in the direction of trade   (remove condition)
  # ------------------------------------------------------------------------------------------------
  ...
  #macd_entry = which(macd_cross & macd12_26_9$MACD < 0 & !(dat_i$close < ema20 & ema20 < ema50 & ema50 < ema100 & ema100 < ema200))
  ...
  for (m in macd_entry) {
    ...
    #crit2 = (dat_i$close[m] - dat_i$close[m-3])/dat_i$close[m]*100 > 5
...
osc = sapply(1:length(macd_trade[-1]),function(x) ifelse(x <= 36,
                                                         (macd_trade[x] - macd_trade[1])/macd_trade[1],
                                                         (macd_trade[x] - macd_trade[x-36])/macd_trade[x]))*100
