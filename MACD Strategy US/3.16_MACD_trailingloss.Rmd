...
macd_strategy = function(dat_i) {
  ...
  # - EMA 200 filter parameters for avoiding ranging markets
  # --------------------------------------------------------------------------------
  ...
  ema200_filter = ema_filter(ema200,dat_i$close,0,0.2)
...
# visualize ema filter
```{r}
df = read.csv(paste0(path,"/AAON.csv"),sep=",",header=TRUE,stringsAsFactors=FALSE)
price = df$close
emaX = ema(price,200)

ind = ema_filter(emaX,price)

emaX[ind] = NA

plot(price[5500:6000],type="l")
lines(emaX[5500:6000],col="red",lwd=2)
```
