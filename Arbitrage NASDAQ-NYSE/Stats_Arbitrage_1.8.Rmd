...
# Compute Potential Trades
```{r}

```

# FROM ETF SCRIPT: 3) Compute Limit Price For Entry and Exit
```{r}
...
for (i in 1:nrow(etf_realtime)) {
  ...
  # get last 20 data
  etf_NZ = etf_NZ[(nrow(etf_NZ)-19):nrow(etf_NZ),]
  etf_id = etf_id[(nrow(etf_id)-19):nrow(etf_id),]
  fx     = fx[(nrow(fx)-19):nrow(fx),]
  ...
  etf_realtime$buy_signal[i]  = tail(crossover(1.5,cum_diff),1)
