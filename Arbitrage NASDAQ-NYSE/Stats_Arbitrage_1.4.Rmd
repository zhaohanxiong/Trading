...
for (n in 1:nrow(pairs)) {
  ...
  for (i in ind) {
    ...
    out_correlation = c(out_correlation, cor_i)
...
# temp = aggregate(out[,c(10,13)], by = list(out$ticker,out$exchange), FUN = mean)
# temp = temp[order(temp$Group.1),]
# temp$n = table(out$ticker)
# View(temp)
```

# Filter Out Low Correlation Stocks
```{r}
# filter by correlation
outt = out[!is.na(out$cor),]
outt = outt[outt$cor <= 0.5,]

# Summarize
ls = list()
ls[[1]] = sprintf("---------------------------")
ls[[2]] = sprintf("Avg Win:  %0.2f%%", mean(outt$return[outt$outcome]))
ls[[3]] = sprintf("Avg Loss: %0.2f%%", mean(outt$return[!outt$outcome]))
ls[[4]] = sprintf("Win Rate: %0.1f%%", sum(outt$outcome)/nrow(outt)*100)
ls[[5]] = sprintf("Expected: %0.2f%%", sum(outt$outcome)*mean(outt$return[outt$outcome])/nrow(outt) + 
                                           sum(!outt$outcome)*mean(outt$return[!outt$outcome])/nrow(outt))
ls[[6]] = sprintf("Avg Hold Period: %0.1f Days", mean(outt$ind.exit-outt$ind.entry))
cat(paste0(ls,collapse="\n"))
```
...
out_real          = outt[order(outt$date.entry),]
out_real$real     = FALSE
